...
public class ClassroomManager {
    ...
    /**
     * 서버로부터 강의실 정보 갱신 (클라이언트 전용)
     * Session의 입출력 스트림을 사용하여 서버에 요청
     */
    public synchronized void refreshFromServer(java.io.PrintWriter out, java.io.BufferedReader in) {
        if (out == null || in == null) {
            System.err.println("[클라이언트 ClassroomManager] 서버 연결 없음 - 로컬 데이터 사용");
            return;
        }
        ...
        try {
            ...
            while (in.ready()) {
                ...
                System.out.println("[클라이언트 ClassroomManager] 버퍼에서 버린 메시지: " + discarded);
            }
            
            // 모든 강의실/실습실 이름 목록 확보
            String[] allRooms = new String[classroomNames.size() + labNames.size()];
            int index = 0;
            for (String name : classroomNames) {
                allRooms[index++] = name;
            }
            ...
            // 각 강의실의 정보를 서버에서 가져오기
            int updatedCount = 0;
            for (String roomName : allRooms) {
                out.println("GET_ROOM_INFO," + roomName);
                out.flush();
                System.out.println("[클라이언트 ClassroomManager] 서버로 요청: GET_ROOM_INFO," + roomName);
                
                // ✅ 응답 대기 - ROOM_INFO로 시작하는 응답을 찾을 때까지
                String response = null;
                int attempts = 0;
                while (response == null && attempts < 50) {  // 최대 5초
                    if (in.ready()) {
                        String line = in.readLine();
                        System.out.println("[클라이언트 ClassroomManager] 받은 응답: " + line);
                        
                        // ✅ ROOM_INFO 응답을 찾을 때까지 다른 응답은 건너뛸
                        if (line != null && line.startsWith("ROOM_INFO:")) {
                            response = line;
                            break;
                        } else {
                            System.out.println("[클라이언트 ClassroomManager] 예상치 못한 응답 건너뛸: " + line);
                        }
                    } else {
                ...
                if (response == null) {
                    System.err.println("[클라이언트 ClassroomManager] " + roomName + " 응답 타임아웃");
                    continue;
                }
                
                System.out.println("[클라이언트 ClassroomManager] 서버 응답: " + response);
                
                if (response.startsWith("ROOM_INFO:")) {
                    String[] parts = response.substring("ROOM_INFO:".length()).split(",");
                    if (parts.length >= 2) {
                        int capacity = Integer.parseInt(parts[0].trim());
                        Classroom classroom = classrooms.get(roomName);
                        if (classroom != null) {
                            int oldCapacity = classroom.capacity;
                            if (oldCapacity != capacity) {
                                classroom.capacity = capacity;
                                updatedCount++;
                                System.out.println(String.format(
                                    "[클라이언트 ClassroomManager] %s 수용인원 서버동기화: %d명 -> %d명",
                                    roomName, oldCapacity, capacity
                                ));
                            } else {
                                System.out.println(String.format(
                                    "[클라이언트 ClassroomManager] %s 수용인원 변경없음: %d명",
                                    roomName, capacity
                                ));
                            }
                        }
                    ...
                    System.err.println("[클라이언트 ClassroomManager] " + roomName + " 오류: " + response);
                } else {
                    System.err.println("[클라이언트 ClassroomManager] " + roomName + " 예상치 못한 응답: " + response);
                }
            }
            
            System.out.println(String.format(
                "[클라이언트 ClassroomManager] 서버 동기화 완료: %d개 업데이트",
                updatedCount
            ));
            
        } catch (IOException e) {
            System.err.println("[클라이언트 ClassroomManager] 서버 동기화 실패: " + e.getMessage());
        } catch (NumberFormatException e) {
            System.err.println("[클라이언트 ClassroomManager] 수용인원 파싱 오류: " + e.getMessage());
        } catch (InterruptedException e) {
            System.err.println("[클라이언트 ClassroomManager] 대기 중 인터럽트: " + e.getMessage());
        }
    }
